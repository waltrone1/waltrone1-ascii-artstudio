...
class SvgBuilder:
    def document(self, rows, title, options: RenderOptions):
        ...
        if show_footer:
            ...
            brand_url_safe = html.escape(BRAND_URL, quote=True)
            parts.append(f'<a href="{brand_url_safe}" target="_blank" rel="noopener noreferrer">')
...
.ascii-export-footer:hover {{
    color: #a8ffc0;
    border-color: #00ff66;
    box-shadow: 0 0 14px rgba(0, 255, 102, .45), 0 0 10px rgba(255, 0, 255, .35), inset 0 0 10px rgba(0, 255, 100, .12);
}}
.ascii-cell {{
...
"""
    ...
    def preview_block(self, ascii_html, raw_ascii, title, options: RenderOptions, root_id="asciiPreview"):
        ...
        if mode == "Bild + Programmname":
            footer_html = f'<a class="ascii-export-footer" href="{html.escape(BRAND_URL, quote=True)}" target="_blank" rel="noopener noreferrer" title="waltrone1 Tools öffnen">{html.escape(SOFTWARE_NAME)}</a>'
...
class RetroAsciiApp:
    ...
    def build_ui(self):
        ...
        self.terminal_canvas.bind("<Button-1>", self.on_terminal_canvas_click, add="+")
    ...
    def draw_internal_preview(self):
        ...
        if mode == "Bild + Programmname":
            ...
            self.brand_badge_bbox = (box_x - 2, box_y - 2, box_x + box_w + 2, box_y + box_h + 2)
        self.draw_arcade_border(c, w, h)

    def open_brand_url(self):
        webbrowser.open_new(BRAND_URL)

    def on_terminal_canvas_click(self, event):
        bbox = getattr(self, "brand_badge_bbox", None)
        if not bbox:
            return
        x1, y1, x2, y2 = bbox
